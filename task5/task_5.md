...
pip install prefect
``` 
Создаем файл `prefect_flow_hw5.py`
```bash
...
def save_table(df: DataFrame, hive_table: str = "titanic_table"):
    df.write.saveAsTable(hive_table, partitionBy="Pclass", mode="overwrite")
...
Теперь создадим файл `prefect_flow_scheduled_hw5.py`, который будем запускать по расписанию
```bash
nano prefect_flow_scheduled_hw5.py
```
...
Устанавливаем корректный API_URL для prefect 
   ```bash
   export PREFECT_URL="http://team-11-nn:port/api"
   ```
Запускаем сервер prefect
   ```bash
   prefect server start --host team-11-nn --port port -b
   ``` 
Установливаем расписание для запуска
   ```bash
   prefect flow serve prefect_flow_scheduled_hw5:spark_flow --cron "0 0 * * *" --name "daily" --no-pause-on-shutdown
   ``` 
